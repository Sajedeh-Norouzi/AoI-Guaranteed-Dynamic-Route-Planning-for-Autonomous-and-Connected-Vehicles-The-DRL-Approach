...
TTlink_path      = os.path.join (current_dir, "model/" +label +'/TT_link.mat')
TTvehicle_path   = os.path.join (current_dir, "model/" +label +'/TT_vehicle.mat')
Agevehicle_path  = os.path.join (current_dir, "model/" +label +'/Age_vehicle.mat')
Flagvehicle_path = os.path.join (current_dir, "model/" +label +'/Flage_vehicle.mat')
start = time.time()
...
agent = Agent(alpha, beta, n_input, update_par, n_output, gamma, memory_size, C_fc1_dims, C_fc2_dims, 
              C_fc3_dims, A_fc1_dims, A_fc2_dims, batch_size, 1)
...
for i in range(aaa-w) :
    temp_value= np.sum(r[i:-aaa + w+i])/w
    mean_ep_rewardall.append(temp_value) 
plt.plot(mean_ep_rewardall, label='A2C: mean rewards')
